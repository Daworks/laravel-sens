...
알림톡을 쓰지 않는다면 이 값은 비워 두어도 됩니다.

> **`1.3.1` 이하를 쓰고 있다면 주의하십시오.**
> 그 버전까지는 이 값이 `null`이면 컨테이너가 클라이언트를 만드는 시점에 `TypeError`가
> 났습니다. 설정 파일을 퍼블리시한 뒤 아래처럼 기본값 없이 두고 환경 변수도 설정하지 않은
> 경우가 여기에 해당합니다.
>
> ```php
> 'alimtalk_service_id' => env('SENS_ALIMTALK_SERVICE_ID'),        // 1.3.1 이하에서 위험
> 'alimtalk_service_id' => env('SENS_ALIMTALK_SERVICE_ID', ''),    // 패키지 기본값
> ```
>
> `1.3.2`부터는 다른 항목과 똑같이 빈 문자열로 처리하므로 이 문제가 없습니다.
